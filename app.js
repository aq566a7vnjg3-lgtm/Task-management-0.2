...
const regPlannedISO = regFixedISO || ((hqArrivalISO && blank) ? calcRegPlannedDate(hqArrivalISO, blank, settings) : null);
  ...
  const garageLead = caseObj.garageLeadBusinessDays ?? settings.defaultGarageLeadBusinessDays;
...
export function topDeadlines(caseObj){
  ...
  if(isOpen('garage_cert_submit')) push('車庫証明', caseObj.derived?.garageCertDueDate);
